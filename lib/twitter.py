...
class TwitterFeed(Twitter):
    ...
    def _rtpostfeed(self, url, delegate, postdata, args):
        if args:
            url += "?" + self._urlencode(args)
        print "Fetching", url
        headers = self._makeAuthHeader()
        ...
        return client.downloadPage(url, txml.HoseFeed(delegate), agent=self.agent,
                                   method='POST',
                                   postdata=self._urlencode(postdata),
                                   headers=headers)
    ...
    def follow(self, delegate, users, args=None):
        """POST to retrieve follow feed."""
        postdata = {'follow': users}
        return self._rtpostfeed("http://stream.twitter.com/follow.xml", delegate, postdata, args)
